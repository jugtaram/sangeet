...
$(document).ready(function(){
        ...
        $.getJSON("http://ws.audioscrobbler.com/2.0/",{method:"tag.getinfo",tag:"<?php echo $name ?>" ,api_key:"key",format:"json"},
...
<script>
$(document).ready(function(){
        $.getJSON("http://ws.audioscrobbler.com/2.0/",{method:"tag.gettoptracks",tag:"<?php echo $name ?>" ,api_key:"key",format:"json"},
    function(responseText){
      $.each(responseText,function(i,fields){
        for(var j=0;j<100;j++){
        var ele=document.createElement("tr");
        $(ele).appendTo("#rest").attr({"you":fields.track[j]["name"]+" "+fields.track[j].artist.name,"son":fields.track[j]["name"]}).addClass("youtube3");
        $("<td></td>").html(j+1).css({"color":"white","text-align":"left","margin-top":"10px"}).appendTo(ele);
          $("<td></td>").html("<b>"+fields.track[j]["name"]+"</b>").css({"color":"white","text-align":"left","margin-top":"10px","cursor":"pointer","padding-right":"0px"}).appendTo(ele);
           $("<td></td>").html("<b>"+fields.track[j].artist.name+"</b>").css({"color":"white","text-align":"left","margin-top":"10px","cursor":"pointer"}).appendTo(ele);
            }
       });
...
<script>
 $(function(){
    ...
    $(document).on('click','.youtube3' ,function(e){
         ...
         let divi=`<div ><ul style="list-style:none;margin-top:10px;">
          ...
          <li style="cursor:pointer"><a data-toggle="modal"  class="viid" data-target="#myyoutube"><button class="btn btn-default" style="background:url(userbg.jpg);margin-top:4px;width:100%;color:white;">See video</button></a></li>
            </ul> </div>`;
       $("#sss").html(" "+divi);
        ...
        $("#like").on("click",function(){
         ...
         e.preventDefault();
              ...
              $(document).on('click','.viid' ,function(e){ 
               e.preventDefault();
              //prepare request
              var request=gapi.client.youtube.search.list({
                     part:"snippet",

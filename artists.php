...
<body>
 ...
 <div id="artz">
   ...
   <ul class="nav nav-tabs" style="background:url(userbg.jpg);margin-top:10px;border-radius:20px">
				<li ><a href="#" id="a1" target="newframe" style="color:orange"><b> Top Albums</b></a></li>
				<li><a href="#" id="a2" style="color:orange"><b>Top Songs</b></a></li>
				<li><a href="#"  id="a3" style="color:orange"><b>About</b></a></li>
...
$(document).ready(function(){
	          let ddd=`<a class="btn btn-default" id="follow2" style="color:white;background:url(userbg.jpg);width:100%;">follow</a>`;
                  $("#follow").html(ddd);
        $.getJSON("http://ws.audioscrobbler.com/2.0/",{method:"artist.getinfo",artist:"<?php echo $name ?>" ,api_key:"key",format:"json"},
    function(responseText){
            ...
            $("<p></p>").attr("fol",responseText.artist.name).html(responseText.artist.name+"</br><pre><b>Listeners</b></pre>   "+responseText.artist.stats.listeners).css({"color":"white","float":"left","margin-top":"50px","font-size":"50px"}).addClass("col-md-2 col-sm-2 set").appendTo("#artistabout");
...
$(document).ready(function(){
    ...
    function(responseText){
       $("#idd").css("display","block");
            $("<h1></h1>").css({"color":"white"}).html(responseText.artist.name).appendTo("#idd");
            $("<p></p>").addClass("col-xs-12 col-lg-12 col-md-12 col-sm-12 new2 ").css({"background":"url(userbg.jpg)","color":"white"}).html(responseText.artist.bio.summary).appendTo("#idd");
	...
	$("#a1").on("click",function(){
     $("#rest").css("display","none");
      ...
      $("#real").css("display","block");
        $.getJSON("http://ws.audioscrobbler.com/2.0/",{method:"artist.gettopalbums",artist:"<?php echo $name ?>" ,api_key:"key",format:"json"},
    function(responseText){
    	$.each(responseText,function(i,fields){
    		
    		for(var j=0;j<fields["@attr"].total;j++){  
    		var pp=document.createElement("span");
    		$(pp).css({"position":"relative","width":"200px","height":"250px","float":"left","border-radius":"20px","background-color":"black","margin":"20px"}).addClass("text-nowrap").appendTo("#real");  		
            $("<span></span>").html("<b>"+fields.album[j]["name"]+"</b>").css({"color":"blue","width":"200px","font-size":"15px","text-align":"left","overflow":"hidden","margin-top":"200px","padding-right":"100px","margin-left":"0px","left":"0px","position":"absolute"}).addClass("text-nowrap").appendTo(pp);
     ...
     $("#real").css("display","none");
      $("#idd").css("display","none");
       $("#rest").css("display","block");
        $.getJSON("http://ws.audioscrobbler.com/2.0/",{method:"artist.gettoptracks",artist:"<?php echo $name ?>" ,api_key"key",format:"json"},
    function(responseText){
    	$.each(responseText,function(i,fields){
...
<script>
 $(function(){
    ...
    $(document).on('click','.youtube2' ,function(e){
      ...
      <li style="cursor:pointer"><a id="like" ><button class="btn btn-default" style="background:url(userbg.jpg);margin-top:4px;width:100%;color:white">Like</button></a></li><li style="cursor:pointer"><a id="playlist"><button class="btn btn-default" style="background:url(userbg.jpg);margin-top:4px;width:100%;color:white;">Add to playlist</button></a></li><li style="cursor:pointer"><a data-toggle="modal"  class="vid" data-target="#myyoutube"><button class="btn btn-default" style="background:url(userbg.jpg);margin-top:4px;width:100%;color:white;">See video</button></a></li>
    </ul> </div>`;
       $("#ss").html(" "+divi);
